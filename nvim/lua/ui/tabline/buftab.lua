...
function Buftab:generate_hl()
  ...
  if options.get()["hlgroup_" .. name] then
    self.hl = options.get()["hlgroup_" .. name]
    return
  end
...
function Buftab:has_icon_colors()
  local icon_colors = options.get().icon_colors
...
function Buftab:generate_icon()
  ...
  self.icon_hl = self:has_icon_colors() and highlights.merge_hl(icon_hl, self.hl) or self.hl
end

function Buftab:new(buf, index, last)
  local t = {}
  t.index = options.get().buffer_id_index and buf.bufnr or index
  t.insert_at = index
  t.last = last

  t.buf = buf
  t.name = vim.fn.fnamemodify(buf.name, ":t")
  t.format = options.get().tab_format
  t.position = "left"
...
function Buftab:highlight()
  ...
  local highlighted = {
    highlights.add_hl(before_icon_part, self.hl),
    highlights.add_hl(icon_part, self.icon_hl),
    highlights.add_hl(after_icon_part, self.hl),
  }
