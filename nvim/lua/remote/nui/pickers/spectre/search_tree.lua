...
local function prepare_node(node, line, component)
  ...
  if ref then line:append("✔ ", component:hl_group "SpectreReplaceSuccess") end
  ...
  if #node.diff.search > 0 then
    ...
    util.foreach(node.diff.search, function(value, index)
      ...
      if index == 1 then line:append(string.sub(code_text, 1, start), default_text_highlight) end
      ...
      if index == #node.diff.search then line:append(string.sub(code_text, end_ + 1), default_text_highlight) end
    end)
  end
